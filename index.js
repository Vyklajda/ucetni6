...
// ── Ceny ─────────────────────────────────────────────────────────────────────
const CENA_ZA_DIL        = 350;  // Nastaveno na fixní cenu z botu ($350 za díl)
const CENA_OPRAVA        = 12000;
const CENA_VYJEZD_MESTO  = 20000;
const CENA_VYJEZD_VENKOV = 40000;
const CENA_TANKOVANI     = 10000;
...
// NOVÉ: Výpočet ekonomicky správné výplaty
function calculateCistaVyplata(emp, koeficient) {
  ...
  emp.tunings.forEach(x => {
    ...
    // Zákazník platí (x.cena), náklady jsou (100 + 35 + 35 = 170)
...
client.on('messageCreate', async (message) => {
  ...
  if (cmd === '!výplata' || cmd === '!vyplata') {
    ...
    const k    = calculateCistaVyplata(emp, koef); // Spočítáno novou metodou
